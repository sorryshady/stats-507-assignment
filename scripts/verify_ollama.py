...
def check_ollama_connection():
    ...
    try:
        ...
        return (
            False,
            f"Cannot connect to Ollama at {OLLAMA_API_URL}. Is Ollama running?",
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            partial_matches = [
                ...
                if "llama3.2" in m.lower() or "llama3" in m.lower()
            ]
            if partial_matches:
                return (
                    False,
                    f"Model {target_model} not found. Available Llama models: {partial_matches}",
                )

            return (
                False,
                f"Model {target_model} not found. Available models: {model_names}",
            )
    except Exception as e:
...
def test_model_inference():
    ...
    try:
        ...
        response = requests.post(
            f"{OLLAMA_API_URL}/api/generate", json=payload, timeout=10.0
        )
