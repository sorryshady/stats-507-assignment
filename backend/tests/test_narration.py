...
def test_narration_endpoint_invalid_frame():
    ...
    response = client.post("/api/narration", json={"frame": "invalid_base64_data!!!"})
...
def test_narration_endpoint_valid_frame():
    ...
    response = client.post("/api/narration", json={"frame": frame_base64})

    # 200 if success, 503 if Ollama not available
