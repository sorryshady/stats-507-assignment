...
async def test_websocket():
    ...
    try:
        async with websockets.connect(uri) as websocket:
            ...
            # Read test image
            test_image_path = os.path.join(os.path.dirname(__file__), "..", "test_images", "test_image_0.jpg")
            ...
            if result.get('hazards'):
                ...
                for hazard in result['hazards']:
                    print(f"    - {hazard['class_name']} ({hazard['priority']}): {hazard['reason']}")
            
            print("\n✅ Test completed successfully!")
            
    except websockets.exceptions.ConnectionRefused:
        print("❌ Connection refused. Is the server running?")
        print("   Start server with: cd backend && uvicorn app.main:app --reload --port 8000")
